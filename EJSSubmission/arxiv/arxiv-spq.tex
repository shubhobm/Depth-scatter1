...
%$p$, for example by choosing $W_{\max} = \exp\{- p^{2} \}$. Such tending to infinity efficiencies 
%are also achievable for fixed $W_{\max}$,  for example, in the context of some hypothesis tests.
%ing problems.
%f $|X - q_{0}|^{2}$ is a Gamma random 
%variable with shape parameter $ = 2 + \exp\{- p^{2} \}$. 
%We leave it to future work to study such efficiencies more generally. 
